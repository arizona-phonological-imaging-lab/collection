# 2/2015 Created by Gus Hahn-Powel
# 2/2015 malign changes and poor coding added by Sam Johnston
...
class Displayer(object):
    ...
    def display_image(self, rotation, img_save_path):
        deg = 0
        print 'WHAT'

        try:
            #rounding necessary; if a non-whole number is passed, the rotation defaults to zero 
            deg = round(rotation)
        ...
        self.new_img = self.current_img.rotate_fan(deg)
        print "face"
        self.current_deg = deg
        self.current_img.show("rotated {} degrees".format(self.current_deg))

        #for Kinect's automatic postprocessing - auto-saving
        cv2.imwrite(img_save_path, self.new_img)
